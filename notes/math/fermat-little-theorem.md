...
计算 $a^b\bmod p$ 时，[快速幂](fast-exponentiation.md) 已经把时间降到 $O(\log b)$。但如果指数有几十万位，连 64 位整数都无法保存，快速幂仍然不能直接接收它。
...
若某个数 $n$ 满足 $a^n\equiv a\pmod n$，不能仅凭这一次检验断定 $n$ 是质数。可靠的素性测试需要额外条件与算法。
...
欧拉定理会把“质数模”推广到一般互质模数；伪素数与 Carmichael 数会说明为什么费马同余不能单独完成确定性素性判定。这些内容不属于本篇基础目标。
