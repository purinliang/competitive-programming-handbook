...
当一个对象由多个相关数据共同描述时，C++ 可以使用 `struct` 声明一种**结构体类型**，将这些数据组合成一个整体。
...
学会 [数组：一维数组](one-dimensional-arrays.md) 后，可以把结构体类型作为数组的元素类型：
...
1. 声明 `struct Point` 类型，使它包含整数成员 `x` 和 `y`；读入一个点后输出 `x + y`。
2. 使用 `Contestant contestants[MAXN]` 读入多个参赛者，输出分数不低于 $60$ 的人数。
3. 在完整代码中先执行 `Contestant winner = contestants[best];`，再通过 `winner` 输出同样的编号和分数。
4. 交换两个 `Contestant` 对象，确认每个对象的 `id` 和 `score` 会一起移动。
...
下一篇 [内存：字节寻址](byte-addressing.md) 会从字节与地址开始解释程序怎样定位一个对象。低频的 `union`、`enum` 与类相关语法统一放在扩展阅读中。
