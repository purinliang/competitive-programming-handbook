...
[有序关联容器：multiset](multiset.md) 可以保存每个重复元素，但查询一个键的 $k$ 个副本时，`count(key)` 需要 $O(\log n+k)$ 时间，并且容器真正保存了这 $k$ 个元素。
...
本篇只学习 C++17 `map` 的常用接口。允许一个键对应多条记录的 `multimap` 将在下一篇单独说明。
...
下一篇 [有序关联容器：multimap](../CATALOG.md#01-c-基础) 会允许同一个键对应多个映射值，并使用等值区间访问其全部记录。
