...
> 最近修订：2026-08-17 10:44 +10:00（未审阅）

[`set`](set.md) 会自动按顺序维护元素，但每个键最多保存一份。有些问题同时需要
有序性和重复次数：
...
若一个值可能重复非常多，而题目又要高频查询其次数，只用 `multiset::count` 可能
会重复遍历整段等值元素。前面学习的 [`map`](map.md) 可以显式保存“数值到次数”
的映射。
...
8. `equal_range(x)` 返回的两个迭代器表示哪个左闭右开区间？
9. 怎样取出并只删除一份当前最小值？
