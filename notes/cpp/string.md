...
> 最近修订：2026-08-17 11:29 +10:00（未审阅）
...
这里的 `auto` 让编译器根据右侧表达式推导 `position` 的类型。它不是一种新的固定
类型；变量初始化以后，推导出的类型也不会在运行中改变。这里使用它，是因为
`find` 返回的是标准库专门用于长度和位置的无符号类型。
...
不要用 `position >= 0` 判断，因为无符号类型本来就不会小于 `0`。使用 `auto`
可以保留 `find` 的正确返回类型，不必在当前阶段额外记住它的完整名称。
