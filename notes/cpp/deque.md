...
`deque` 是 double-ended queue 的缩写，中文称为双端队列。它不是下一篇 `queue` 的另一种写法：`deque` 可以遍历、随机访问并修改两端，`queue` 则是故意只暴露先进先出接口的容器适配器。
...
下一篇 [容器适配器：queue（正文待写）](../CATALOG.md#01-c-基础) 会把 `deque` 限制为只允许队尾加入、队首查看和删除，从而直接表达先进先出。
