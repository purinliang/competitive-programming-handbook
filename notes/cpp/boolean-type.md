...
> 最近修订：2026-08-13 01:53 +10:00（未审阅）
...
假设算法开始时还没有访问节点 $3$，可以把对应标记初始化为 `false`：
...
这说明 `bool` 不只是手动写入的标记，也是程序进行判断后得到的结果。各种比较符号的完整规则会在后面的“表达式：比较运算符”中解释；本篇先使用最直观的 `==`。
...
这两种写法表达同一个判断。需要在后面重复使用结果时保存为变量，只使用一次时可以直接写条件。[控制流：条件分支](conditional-branches.md) 会完整解释 `if` 的执行过程。
...
下一篇 [变量：声明与初始化](variable-declaration-initialization.md) 会整理此前已经反复使用的变量写法，区分声明、初始化、赋值和未初始化状态。
