...
> 最近修订：2026-08-16 21:33 +10:00（未审阅）
...
在主流竞赛环境中，`sizeof(bool)` 通常是 `1`，也就是占用一个字节：

```cpp
cout << sizeof(bool) << '\n';
```

这不表示 `bool` 能保存一个字节里的所有整数。对象占用多少存储空间，与类型允许
程序观察到多少种值是两个问题；`bool` 仍然只表示 `false` 和 `true`。

### 连续两次逻辑取反

逻辑非运算符 `!` 会把真假反转。对整数使用时，`!x` 在 `x == 0` 时为 `true`，
在 `x != 0` 时为 `false`。因此连续写两次：

```cpp
bool flag = !!x;
```

会把任意整数规范成对应的布尔值：`0` 变成 `false`，非零值变成 `true`。这只是
整数转布尔规则的紧凑写法，不是必须背诵的技巧；直接写清楚比较通常更容易读。
[逻辑运算符](logical-operators.md) 会继续解释 `!`、`&&` 和 `||`。
...
这两种写法表达同一个判断。需要在后面重复使用结果时保存为变量，只使用一次时可以直接写条件。[条件分支](if-and-else.md) 会完整解释 `if` 的执行过程。
...
8. `sizeof(bool)` 通常是 `1`，为什么这不表示它能保存 256 种逻辑值？
9. `!!x` 会把整数 `x` 规范成怎样的布尔值？
