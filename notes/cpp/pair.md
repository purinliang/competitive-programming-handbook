...
这个规则对按“主关键字，再按次关键字”排序非常方便。具体排序接口会在 [STL 算法：排序（正文待写）](../CATALOG.md#01-c-基础) 中使用这个性质。
...
下一篇 [工具类型：tuple（正文待写）](../CATALOG.md#01-c-基础) 会把固定数量的成员从两个扩展到三个或更多，并继续使用结构化绑定为它们命名。
