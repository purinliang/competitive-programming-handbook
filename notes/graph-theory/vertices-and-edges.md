...
点权的意义由具体问题决定。程序中通常使用 `value[u]`、`weight[u]` 或题目指定的数组保存点 $u$ 的点权。
...
一条边的两个端点也可能是同一个点，这种特殊的边称为自环（self-loop，也常简称 loop）。从环的角度看，自环同时也是只含一个点和一条边的特殊环；下一篇会继续解释环。
...
下一篇将介绍 [图：路径与环](paths-and-cycles.md)。
