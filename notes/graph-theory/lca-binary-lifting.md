...
在 [树：有根树](rooted-trees.md) 中，每个节点都有唯一的父节点链一直通向根。给定两个节点 `u` 和 `v`，树上路径、距离、差分和很多子树问题都需要找到它们向上走时第一次汇合的节点。
...
- 两条链的公共节点有 $2$ 和 $1$，其中 $2$ 的深度更大，所以 $\operatorname{LCA}(5,6)=2$。
...
按当前学习路线，下一篇是 [图的存储：边集](edge-list.md)。
