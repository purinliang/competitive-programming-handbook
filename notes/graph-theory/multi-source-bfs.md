...
若一个连通块中没有任何起点，该块中的点不会入队，搜索结束后继续满足 `dist[u] == -1` 和 `nearest[u] == -1`。
...
普通 BFS 的分层依赖每条边代价相同，因此本文直接求的是无权图中的最少边数。若边权只可能为 `0` 或 `1`，下一篇的 `0-1 BFS` 会使用 `deque`；若边权是任意非负数，需要使用 Dijkstra。它们也可以把多个给定起点都初始化成距离 `0`，但更新规则不再是本文的普通队列 BFS。
...
2. 给图增加一个没有起点的新连通块，确认其中所有点的两个输出都为 `-1`。
...
下一篇 [最短路：0-1 BFS](../CATALOG.md#04-图论) 会允许边权为 `0` 或 `1`，并根据新距离是否增加一条单位代价决定从 `deque` 的哪一端加入状态。
