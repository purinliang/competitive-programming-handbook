...
1. 把完整程序改成读取无权有向图，删除边记录中的 `w`。
2. 把输入解释为无向图，但每条原始边仍只保存一次；交换一条记录中的 `u` 和 `v`，确认它表示的无向边没有改变。
3. 统计未经索引的边集中从指定点 `start` 出发的边，并记录实际检查了多少条边。
...
需要以边编号直接定位反向边时，可以使用邻接表的数组实现：
[图的存储：邻接表（链式前向星实现）](chained-forward-star.md)。
