...
DFS 总是按照 `g[u]` 中邻接项的先后顺序选择下一条边。交换输入边的顺序可能改变访问顺序和 DFS 树，却不会改变哪些点彼此连通，也不会改变连通块数量。
...
1. 删除统计连通块的外层循环，只从指定起点 `start` 开始 DFS，输出所有从它可达的点。
...
[DFS、回溯与剪枝](dfs-backtracking-pruning.md) 会把逐步选择产生的状态看成一棵隐式搜索树，并在递归返回时撤销当前选择。
