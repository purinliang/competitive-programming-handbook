...
> 最近修订：2026-08-16 12:52 +10:00（未审阅）

在 [点与边](vertices-and-edges.md) 中，我们只记录了哪些点之间有边。沿着首尾相接的边从一个点走到另一个点时，经过的点和边会形成图中最常用的两个结构：路径与环。
...
其中 $v_0,v_1,\ldots,v_{k-1}$ 两两不同。下图中的红色部分依次经过点 `1, 2, 3, 1`，是一个环。

![路径、环与自环](../assets/graph-theory/path-cycle-self-loop.svg)

自环是一个只有一个点和一条边的特殊环。上图的点 `5` 上就有一个自环；从边的角度看，它同时也是两个端点相同的特殊边。
...
1. 在一张无向图中写出一条含四个点的路径，并说明它的长度。
2. 给一串重复经过中间点的边，判断它为什么不是本篇定义下的路径。
3. 分别画出一个三点环和一个自环，说明二者在点数、边数上的区别。
4. 给一条有向边序列，逐边检查箭头方向，判断它能否组成有向路径。
5. 构造一个允许重复点但不重复边的闭合结构，说明它为什么是回路却不是环。
...
[度数](vertex-degrees.md) 会从另一角度统计每个点关联了多少条边。
