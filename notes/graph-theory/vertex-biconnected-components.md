...
《[无向图：割点与桥](articulation-points-bridges.md)》已经用 `dfn` 与 `low`
判断哪些点或边是无向图的薄弱连接。只知道割点仍不能回答“删除这个点会把哪些区域
分开”：我们还需要把不能再由单个点切开的部分完整划分出来。

这些极大的部分称为点双连通分量。不同点双连通分量可以共享割点，却不会共享普通点。
本文直接从割点算法的 DFS 信息继续，重点解释怎样用点栈取出每个分量。
...
输入一个无自环的无向图，可以含重边。程序输出割点和所有点双连通分量。
...
Tarjan DFS 中每条无向边只被常数次访问。每条 DFS 树边只在某次分量闭合时对应一次
弹栈，总时间和空间复杂度均为 $O(n+m)$。
