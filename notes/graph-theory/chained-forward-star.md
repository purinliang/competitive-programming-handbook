...
```cpp
for (int edge_id = head[u]; edge_id != -1; edge_id = edges[edge_id].next) {
...
`MAXM` 必须覆盖展开后的边记录数。这里假设输入无向边数量不超过 `200000`，每条无向边展开成两条记录，所以令 `MAXM = 2 * MAXN`；`MAXN` 自带的 `+5` 余量也随关系一起保留下来。
...
```text
1: (3, 7, edge 2) (2, 4, edge 0)
2: (3, 5, edge 4) (1, 4, edge 1)
3: (2, 5, edge 5) (1, 7, edge 3)
```

每个点的出边顺序与输入顺序相反；边编号 `0` 与 `1`、`2` 与 `3`、`4` 与 `5` 分别互为配对记录。
...
1. 根据图示的 `head` 和 `next`，分别手动写出点 $1,2,3$ 的完整出边链。
2. 删除 `weight` 字段和参数，把完整代码改成无权无向图版本。
3. 连续加入若干对边，检查每个编号 `i` 是否满足 `(i ^ 1) ^ 1 == i`，再尝试在一对记录之间插入无关边，观察约定为何失效。
...
[欧拉问题：路径、回路与图](eulerian-paths-and-circuits.md) 会使用成对边和表头推进，
构造恰好使用每条原始边一次的路径或回路。
