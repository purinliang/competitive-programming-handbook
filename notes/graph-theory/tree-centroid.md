...
[树：直径与中心](tree-diameter-center.md) 选择一个节点，使它到最远节点的距离尽量小。现在换一种“平衡”要求：若删除一个节点以及与它相连的边，原树会分成若干连通块；我们希望其中最大的连通块尽量小。
...
最大连通块大小为 `3`。树共有 `7` 个节点，而 `3` 不超过一半，所以节点 `2` 是这棵树的重心。

同一棵树的直径可以是 `6 - 2 - 3 - 4 - 5`，中点是节点 `3`，因此它的中心是 `3`，重心却是 `2`。这个例子直接说明两种概念衡量的目标不同。

## 重心的定义

对于每个候选节点 `u`，删除它以后，令 `balance[u]` 表示剩余各个连通块大小的最大值。

树的重心就是令 `balance[u]` 最小的节点。等价地，节点 `u` 是重心，当且仅当删除它以后每个连通块都不超过整棵树的一半：
...
可以依次尝试删除每个节点，再用 BFS 或 DFS 统计剩余连通块：

```text
n 个候选节点 × 每次 O(n) = O(n²)
```

问题在于每次删点都重复计算了大量相同部分。任选节点 `1` 为根以后，一次 DFS 就能求出每个节点的子树大小；这些大小足以计算删除任意节点后的全部连通块。
...
对于 `u` 的每个子节点 `v`，删除 `u` 会切断边 `u - v`。以 `v` 为根的整棵子树成为一个独立连通块，大小恰好是：
...
维护目前见过的最小最大连通块 `best_balance`，以及达到它的全部节点 `centroids`。
...
若删除当前节点 `u` 后存在一个大小超过 `n / 2` 的连通块，那么其他所有连通块加上 `u` 自己的总大小都小于它。沿着连接这个大连通块的边移动一步，会把原来的其余部分变成较小的一侧，因此不会错过更平衡的位置。

不断向唯一可能超过一半的连通块移动，不可能在树中绕环，最终会停在一个没有任何剩余连通块超过一半的节点；它就是重心。这也说明重心一定存在。
...
下面读入一棵树，第一行输出删除重心后最大连通块大小的最小值，第二行输出全部重心：
...
删除节点 `2` 后最大连通块含 `3` 个节点，并且没有其他节点能取得更小值。

## 正确性

DFS 返回时，`subtree_size[v]` 已经准确表示每个子节点方向的连通块大小。删除 `u` 后，这些子树分别成为连通块；剩下不在 `u` 子树中的节点全部连通，大小为 `n - subtree_size[u]`。算法取这些值的最大值，因此准确得到 `balance[u]`。
...
只比较 `subtree_size[v]` 会遗漏 `n - subtree_size[u]`。对于靠近叶节点的候选，这个上方部分通常才是最大的连通块。
...
中心最小化最远距离，重心最小化最大剩余连通块大小。两者的算法、答案和应用都可能不同。
...
6. 随机生成小树，逐个删除节点并用 BFS 统计连通块，与线性算法对拍。
...
4. 删除 `u` 后，每个子节点方向的连通块有多大？
5. 父节点方向的连通块为什么是 `n - subtree_size[u]`？
...
- [点分治（正文待写）](../catalog.md#04-图论) 会反复选择当前连通块的重心作为分治点，使子问题规模至多减半。

## 下一篇

下一篇 [编码：哈夫曼编码](../algorithm-basics/huffman-coding.md) 会从等长编码的浪费出发，用二叉树为高频符号分配更短的前缀编码。
