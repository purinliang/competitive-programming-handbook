...
[图的遍历：深度优先搜索（DFS）](graph-depth-first-search.md) 使用 `visited` 防止沿环重复访问。[树：有根树](rooted-trees.md) 则说明树没有环，任意两个节点之间又只有一条路径；指定根以后，只要跳过刚刚经过的父节点，就能让同一套搜索自然地沿父子层级遍历整棵树。
