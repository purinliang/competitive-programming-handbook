...
下面的程序在至少包含 $3$ 个点的简单无向图中寻找一条哈密顿回路。搜索固定从点 $1$ 开始；若成功，`path` 会在末尾再次加入点 $1$。
...
搜索恰好覆盖五个点，并检查最后一个点 $5$ 能够回到起点 $1$。边 $(1,3)$ 没有被使用，不影响答案成立。
...
代码中的 `MAXN = 25 + 5` 只是为点编号预留数组容量，不表示最坏情况下能够搜索到 $25$ 个点；实际可行规模高度依赖图的结构和剪枝。这份实现只适合点数较小的题目。学习状压 DP 后，可以用“已经使用的点集 + 当前终点”表示状态，把某些哈密顿问题改写成 $O(n^2 2^n)$ 的算法。

## 基础练习

1. 手动模拟样例中 `path` 和 `used` 的变化，标出搜索成功后哪些递归调用不会再执行撤销操作。
2. 把终止条件改成寻找哈密顿路径，并依次尝试每个点作为起点；每次更换起点前需要清空哪些状态？
3. 输入一条包含 $5$ 个点的链，观察回溯为什么找不到哈密顿回路，再用上一题的修改找到一条哈密顿路径。
