...
广度优先搜索（breadth-first search，BFS）从起点开始，先处理距离起点一条边的点，再处理距离两条边的点，依次向外扩展。图可能有环，同一个点还可能从多条边被发现；图也可能不连通，从一个起点无法到达所有点。因此 BFS 需要访问标记，并且一次搜索只会处理从起点可达的部分。
...
下一篇将从一般图进入它的特殊形态，介绍 [树：无根树](unrooted-trees.md)。
