...
0-1 BFS 专门处理边权只含 `0` 和 `1` 的最短路。它通过 [序列容器：deque](../cpp/deque.md) 同时操作队列两端，让当前距离较小的状态始终先处理，时间复杂度仍然是 $O(n+m)$。
...
下一篇 [二分图：判定](../CATALOG.md#04-图论) 会尝试用两种颜色为无向图的点分类，并用 BFS 或 DFS 检查每条边是否连接异色点。
