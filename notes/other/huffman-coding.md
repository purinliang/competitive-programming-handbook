...
本篇需要已经了解 [完全二叉树](../data-structures/complete-binary-tree.md)、
[二叉堆](../algorithm-basics/binary-heap.md)、[`priority_queue`](../cpp/priority-queue.md)
和 [二叉树的存储](../data-structures/binary-tree-storage.md)。
...
考虑一棵最优的二进制前缀编码树。没有必要保留只有一个子节点的内部节点，因为删掉
它能让其下全部叶节点变浅一层。因此可以只考虑每个内部节点都有两个子节点的完整
二叉树结构。
