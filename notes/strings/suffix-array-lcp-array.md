...
比较子串 $s[l_1..r_1]$ 和 $s[l_2..r_2]$ 时，先查询从 $l_1$ 和 $l_2$ 开始的两个后缀的最长公共前缀，再把结果限制在较短子串的长度以内。

- 若公共部分已经覆盖两个子串，并且它们长度相等，那么两个子串相等；
- 若公共部分只覆盖较短子串，那么较短者的字典序更小；
- 否则，比较公共部分之后的第一个不同字符即可判断字典序。

因此，静态字符串中的子串相等判断和字典序比较都可以复用同一套“后缀 LCP + 区间最小值”结构，不需要逐个字符重新扫描。

### 按公共前缀分组

固定一个长度 $length$。若 `lcp[order] >= length`，排名相邻的两个后缀拥有相同的长度为 $length$ 的前缀。把所有满足条件的相邻位置连接起来，后缀数组就会被划分成若干连续块；同一块中的后缀都以同一个长度为 $length$ 的字符串开头。

一个包含 $count$ 个后缀的块，表示对应子串至少出现了 $count$ 次。于是可以据此统计重复子串的出现次数，也可以判断是否存在至少出现若干次、长度至少为某个值的子串。不同题目只是改变 $length$、块大小和答案恢复方式，连续块的来源始终是同一个 `lcp` 条件。
