...
| 0238 | 邻项交换证明 | 算法基础 | `algorithm-basics/greedy-adjacent-exchange.md` |
...
| ID | 知识点 | 模块 | 文件 |
| --- | --- | --- | --- |
| 0433 | 无根树 | 图论 | [graph-theory/unrooted-trees.md](graph-theory/unrooted-trees.md) |
| 0442 | 有根树 | 图论 | [graph-theory/rooted-trees.md](graph-theory/rooted-trees.md) |
...
| ID | 知识点 | 模块 | 文件 |
| --- | --- | --- | --- |
| 0311 | 二叉树的结构与存储 | 数据结构 | [data-structures/binary-tree-structure-and-storage.md](data-structures/binary-tree-structure-and-storage.md) |
...
以下独立扩展不属于 01–06 的必学顺序，其中一部分可能明显高于高中竞赛一等奖的常见范围。它们按模块和 ID 排列，便于查找；开始某篇之前可以回到 [模块目录](catalog.md) 查看状态和所属模块。使用 `e1*`、`e2*` 等编号的附属扩展只从对应基础文章进入，不在这里重复平铺。
