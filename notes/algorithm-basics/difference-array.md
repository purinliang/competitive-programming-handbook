...
> 最近修订：2026-08-13 20:56 +10:00（未审阅）
...
给定一个长度为 $n$ 的 1-based 数组 `a[1..n]`，接下来有 $m$ 次修改。每次给出 `l`、`r` 和 `delta`，把闭区间 `[l, r]` 中的每个元素都增加 `delta`。全部修改结束后，输出最终数组。
...
`difference` 按统一约定分配 `n + 5` 个位置，所以 `n + 1` 是安全的边界哨兵。最终只恢复 `1..n`，不会读取它。这样所有修改都使用同样的两行代码，不需要为 `r == n` 增加分支。
...
1. 对 `3 1 4 1 5 9` 手算初始差分，并用前缀累加恢复原数组。
2. 手动执行 `[2, 5] += 4`，解释为什么只修改下标 `2` 和 `6`。
3. 测试 `l == r` 的单点区间、`l == 1`、`r == n`、负增量和零增量。
4. 用逐元素区间加与差分随机对拍，比较全部修改后的数组。
5. 从全零数组开始，只根据若干次区间加建立最终数组。
6. 统计每个位置被多少个给定区间覆盖：把每个区间视为增加 $1$。
7. 构造会让最终值超出 32 位整数的修改序列。
...
下一篇 [贪心：选择与证明](greedy-selection-and-proof.md) 会研究怎样用局部选择构造整体答案，以及必须怎样证明选择不会排除最优解。
