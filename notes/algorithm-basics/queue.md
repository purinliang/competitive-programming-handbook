...
[queue](../cpp/queue.md) 会使用标准库提供的同名接口。本篇先推导这些操作怎样实现以及为什么都是 $O(1)$。
...
数组和链表回答“元素怎样存储和连接”，栈和队列回答“允许从哪些端操作”。同一种底层结构可以实现不同的访问规则。两端都能加入和删除的结构是另一个独立主题，不在普通队列中提前展开。
...
双端队列、优先队列、单调队列、多级反馈调度以及并发无锁队列不属于本篇基础目标。普通广度优先搜索会在图的遍历文章中把队列用于逐层扩展状态。
