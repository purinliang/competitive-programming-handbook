...
阶段 3 的 [容器适配器：queue（正文待写）](../CATALOG.md#01-c-基础) 会使用标准库提供的同名接口。本篇先推导这些操作怎样实现以及为什么都是 $O(1)$。
...
循环数组和双向链表都可以实现双端队列。标准库 [序列容器：deque（正文待写）](../CATALOG.md#01-c-基础) 会教授比赛中直接使用的接口，单调队列等算法则会说明为什么确实需要同时操作两端。
...
下一篇 [排序：基础排序（正文待写）](../CATALOG.md#02-基础算法) 会真正推导若干简单排序算法，而不只把 `sort` 当作现成接口使用。
