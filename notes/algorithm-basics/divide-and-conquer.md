...
> 最近修订：2026-08-13 21:16 +10:00（未审阅）

[排序：快速排序](quicksort.md) 和 [排序：归并排序](merge-sort.md) 都把一个区间变成规模更小的同类区间，再递归处理。这里把它们共同使用的思路单独抽出来：分治（divide-and-conquer）。
...
这里没有在函数开头一次定义所有变量。每个量都只在相应步骤需要它时出现：先有左右子问题，才需要 `left_answer` 和 `right_answer`；发现跨中点情况缺失以后，才引入 `left_suffix`、`right_prefix` 和 `cross_answer`。
...
快速排序的主要工作发生在递归之前：先分区，再解决两侧。归并排序和本例的主要组合发生在递归之后：先得到子问题答案，再合并。分治不要求所有算法具有完全相同的代码顺序，但必须保证子问题足够小、最终可终止，并且原问题的所有答案情况都被覆盖。
...
1. 手动计算数组 `-5 -2 -7` 的每层左右答案与跨中点答案，确认最终返回 `-2`。
2. 用 $O(n^2)$ 的枚举程序与分治程序随机对拍。
3. 修改函数，使它同时返回最大和对应的左右端点；规定存在多解时怎样选择。
4. 构造一个答案完全位于左半区、一个完全位于右半区、一个跨过中点的样例。
5. 分别分析“合并需要 $O(1)$”“合并需要 $O(n)$”时，均匀二分递归树每层的工作量。
6. 回顾快速排序和归并排序，指出各自的划分、子问题、终止条件和合并方式。
...
下一篇 [枚举（正文待写）](../CATALOG.md#02-基础算法) 会系统整理怎样不重不漏地生成候选，并用约束逐步减少需要检查的情况。
