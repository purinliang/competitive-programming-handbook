...
[线性结构：数组](array.md) 把元素连续存放，因此能在 $O(1)$ 时间内通过下标访问；但在中间插入或删除时，后缀元素必须整体移动。
...
下面的程序用索引实现一个单向链表。输入和 [线性结构：数组](array.md#完整代码) 的示例相同：先建立初始序列，在指定逻辑位置插入一个值，再删除插入后的一个逻辑位置。
...
下一篇 [栈（正文待写）](../CATALOG.md#02-基础算法) 会限制只能在同一端加入和删除元素，从而得到后进先出的访问顺序。
