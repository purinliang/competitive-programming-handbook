...
[树的遍历：深度优先搜索（DFS）](../graph-theory/tree-depth-first-search.md) 会在进入
一个节点后递归处理它的每棵子树。[二叉树的存储](binary-tree-storage.md) 进一步
显式保存了左子树和右子树，因此每个节点都有三个稳定的处理位置：递归左子树之前、
左右子树之间、递归右子树之后。把“处理当前节点”分别放在这三个位置，就得到前序、
中序和后序遍历。
...
这棵示例树恰好还是 [二叉搜索树](binary-search-tree-construction.md)，所以中序遍历的值递增。递增不是任意二叉树都具有的性质，而是二叉搜索树的左小右大规则带来的结果。
...
> 当二叉树退化成只有单侧孩子的长链时，递归深度可能超过评测环境的调用栈限制。三种遍历都可以改用显式栈，但非递归写法的状态管理各不相同，不在本篇基础主线中展开。

## 基础练习

1. 手算交换示例树根节点的左右子树之后，三种遍历序列分别怎样变化。
2. 把三种函数中的输出改成把节点值加入三个 `vector<int>`，最后统一输出序列。
3. 在后序位置根据左右子树的结果计算每个节点的子树大小或高度。
