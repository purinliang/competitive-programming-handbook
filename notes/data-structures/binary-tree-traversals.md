...
[树的遍历：深度优先搜索（DFS）](../graph-theory/tree-depth-first-search.md) 会在进入一个节点后递归处理它的每棵子树。[二叉树：结构与存储](binary-tree-structure-and-storage.md) 进一步规定了左子树和右子树，因此每个节点都有三个稳定的处理位置：递归左子树之前、左右子树之间、递归右子树之后。把“处理当前节点”分别放在这三个位置，就得到前序、中序和后序遍历。
