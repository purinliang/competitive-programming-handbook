...
- 第一版只让学习路线出版版本参与进度和讨论。模块目录正文继续公开查阅，但不与学习路线共享状态。
...
写接口需要登录、Origin 检查、请求体上限、速率限制和 Turnstile 风险控制。Cookie 使用 `HttpOnly`、`Secure`、`SameSite=Lax`。公开评论输出前由服务端完成匿名化，不能把真实用户对象发送到浏览器以后再隐藏。
