...
现有 Markdown 知识库还将逐步发布为完全基于 Cloudflare 技术栈的免费在线学习平台。网站的产品范围、内容身份、用户数据边界和阶段计划见 [在线学习平台规划](docs/WEB-PLATFORM.md)。
...
`notes/` 负责保存知识库、学习路线、训练计划、复盘、教学材料、错误经验和回忆录。知识正文按功能或领域存放；`catalog.md` 维护模块目录，`learning-path.md` 维护教学顺序。模块正文与学习路线正文允许按教学需要分离，Markdown 和后续 TeX 编译应引用同一套图片资产。
