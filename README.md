...
`templates/` 下的可复用模板实现优先使用 `.hpp`，验证入口和题目接口使用 `.cpp`。

- `.hpp`：放可 include 的模板实现。这里的“实现”是 header-only 竞赛库实现，不是工程里的独立编译单元。
- `.cpp`：放具体题目的验证入口，负责读取输入、调用模板、输出答案，也可以包含题目特有的适配代码和胶水代码。
